...
<script>
import {NewCompetenceLight} from 'models/competence';
...
export default {
  ...
  data() {
    return {
      ...
      newCompetence: new NewCompetenceLight(0,0,'','')
    };
  },
  components: {
  },
  methods: {
    save() {
      this.newCompetence.accountId = this.account
      this.$validator.validateAll().then(isValid => {
        if(isValid) {
          this.$store.dispatch('competences/addCompetence', this.newCompetence).then(
            () => {
              eventBus.dispatch("newCompetenceSubmitted"); 
            },
            error => {
              this.loading = false;
              this.message =
    ...
    clear() {
      eventBus.dispatch("newCompetenceCancelled"); 
    },
    processSelected() {
      this.newCompetence.processId = this.selectedProcess;
